...
class MarketAnalyzer:
    ...
    def calculate_signals(self, df: pd.DataFrame) -> dict:
        """Calculate technical signal strength with data validation"""
        try:
            # Validate input data
            if df.empty or len(df) < 20:
                logger.warning("Insufficient data for signals")
                return {'signal_strength': 0.5}  # Neutral fallback
            
            # Calculate indicators
            rsi = talib.RSI(df['close'], timeperiod=14).iloc[-1]
            macd_line, signal_line, macd_hist = talib.MACD(df['close'])
            macd_hist = macd_hist.iloc[-1]  # Get scalar value
            
            # Handle division by zero in volume calculation
            vol_ma = df['volume'].rolling(20).mean().iloc[-1]
            current_vol = df['volume'].iloc[-1]
            volume_strength = 0.0
            
            if vol_ma > 0:
                volume_strength = min(1, current_vol / (vol_ma * 2))
            
            # Calculate strengths safely
            rsi_strength = max(0.0, min(1.0, (rsi - 30) / 40)) 
            macd_strength = min(1.0, abs(macd_hist) / 0.05)  # Cap at 1.0
            
            # EMA calculations (optional, only if needed elsewhere)
    ...
    def check_volatility(self, df: pd.DataFrame) -> bool:
        """Check if asset meets volatility requirements using ATR"""
        try:
            atr = talib.ATR(df['high'], df['low'], df['close'], 14).iloc[-1]
            current_price = df['close'].iloc[-1]
            return (atr / current_price) > 0.015  # 1.5% volatility threshold
        except:

            return False

    def check_trend_strength(self, df: pd.DataFrame) -> bool:
        """Validate trend strength using MACD and RSI"""
        try:
            macd = talib.trend.MACD(df['close']).macd_diff().iloc[-1]
            rsi = talib.RSI(df['close'], 14).iloc[-1]
            return (macd > 0) and (30 < rsi < 70)
        except:
